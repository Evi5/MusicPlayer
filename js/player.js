...
class PlayerCreator {
    constructor() {
        ...
        this.lyrics = null
        this.getLyrics = (url, id, title, offset, filename) => {
            // 隐藏tab按钮
            ...
            .then(data => {
                if (!this.lyrics) {
                    this.lyrics = new LrcHandler();
                }
                if (offset === 404 || offset === null)
                    offset = 0
                this.lyrics.init(this, data, id, title, offset, filename); // 这里的this将指向正确的上下文
                // 显示tab按钮
    ...
    init() {
        ...
        this.listContent = document.querySelector('.music__list_content');
        let firstListItem = document.querySelector('.music__list_content .list-item');
        this.itemHeight = firstListItem.offsetHeight;
    }
    // 使用fetch API从服务端获取音乐数据
    fetchMusics() {
        document.getElementById('loadingIcon').style.display = 'block';
        return fetch('http://localhost:5000/getAllMusicInfo')
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
            ...
            .then(data => {
                // 将获取到的歌曲数据创建Musics实例，并赋值给this.musics
                this.musics = new Musics(data);
                this.init(); // 音乐数据加载完毕后初始化播放器
                document.getElementById('loadingIcon').style.display = 'none';
            });
    }
    //生成播放列表
    renderSongList() {
        ...
        this.musics.songs.forEach((song, i) => {
            if (song.offset === null){
                _str += `<li class="music__list__item list-item">${song.title}</li>`
            }else if(song.offset < 0){
                _str += `<li class="music__list__item list-item" style="color: #25F9EC;">${song.title}</li>`
            }else if(song.offset == 0){
                _str += `<li class="music__list__item list-item" style="color: lime;">${song.title}</li>`
            }else if(song.offset == 404){
                _str += `<li class="music__list__item list-item" style="color: red;">${song.title}</li>`
            }else if(song.offset > 0){
                _str += `<li class="music__list__item list-item" style="color: #95F925;">${song.title}</li>`
            }
        });
    ...
    renderSongStyle() {
        ...
        this.getLyrics(lyricsUrl, id, title, offset, filename)
    ...
    changeSongIndex(type) {
        this.lyrics.updateLyrics(this.song_index)
        if (typeof type === 'number') {
            ...
            if (this.loop_mode === 0) {
                ...
                if (this.song_index >= 12) {
                    this.listContent.scrollTop += type === 'next' ? this.itemHeight : -this.itemHeight;
                    console.log(`player.scrol:${this.listContent.scrollTop}`)
                }
...
class Progress {
    ...
    drag() {
        ...
        let dragUp = () => {
            flag = false;
            $(document).off('mousemove', this.dragFun)
